  ...
  if (cur != idle_thread)
    {
      ...
      int decay = DIVIDE (load, ADD_INT (load, 1));
      cur->recent_cpu = ADD_INT (MULTIPLE (decay, cur->recent_cpu), cur->nice);
    }
}
void calculate_load_avg (void)
{
  ...
  load_avg = MULTIPLE (DIV_INT (CONVERT_TO_FP (59), 60), load_avg) + MULT_INT (DIV_INT (CONVERT_TO_FP (1), 60), ready_threads);
}
...
static bool sleep_threads_less(const struct list_elem *first, const struct list_elem *second)
{
  ASSERT (first != NULL);
  ASSERT (second != NULL);
  const struct thread *t1 = list_entry (first, struct thread, elem);
  const struct thread *t2 = list_entry (second, struct thread, elem);
